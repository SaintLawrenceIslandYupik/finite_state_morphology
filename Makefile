...
test-ch3: tests/jacobson_examples/jacobson_ch3.tsv ess.fomabin
		@cut -f 1 tests/jacobson_examples/jacobson_ch3.tsv | sort -d -f | uniq | flookup -i -w "" ess.fomabin | sort -d -f | diff - tests/jacobson_examples/jacobson_ch3.tsv && echo "Jacobson (3001) Ch3 - PASS" || echo "Jacobson (2001) Ch3 - FAIL"

test-ch4: tests/jacobson_examples/jacobson_ch4.tsv ess.fomabin
		@cut -f 1 tests/jacobson_examples/jacobson_ch4.tsv | sort -d -f | uniq | flookup -i -w "" ess.fomabin | sort -d -f | diff - tests/jacobson_examples/jacobson_ch4.tsv && echo "Jacobson (4001) Ch4 - PASS" || echo "Jacobson (2001) Ch4 - FAIL"
