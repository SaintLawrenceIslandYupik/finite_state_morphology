...
def print_suffixes(inflClass, jacobson_notation_suffixes, class_specific_suffixes, rootType):
    ...
    for i, suffixWithDefinition in enumerate(annotatedSuffixes):
        if suffixWithDefinition:
            suffix, definition = suffixWithDefinition.split(",")
            # ^^^ CLASSIFY SUFFIX
            jacobsonSuffix = re.sub('-|$', '', jacobsonSuffixes[i].split(",")[0])

            print(jacobsonSuffix + ":^" + suffix)
            # ^^^ CALCULATE PADDING BASED OFF THE ABOVE STRING
    print()
...
def main():
    ...
    parser.add_argument('--root_type', '-r', type=str, help = 'type of root: either Noun or Verb')
    args = parser.parse_args()

    # annotate each suffix for each inflection class
    class2suffixes = annotate_suffixes(args.suffixes, args.root_type)
    ...
    for inflectionClass in class2suffixes.keys():
        ...
        if inflectionClass > 0:
            print_suffixes(inflectionClass, class2suffixes[0], class2suffixes[inflectionClass], args.root_type)
