...
rules_for_noun = [{"%{E%}": "", "(ng)": "ng"}, # NounInflI: when suffixing to noun roots that end in -a, -i, -u, -aa, -ii-, -uu
                  ...
                  {"%{E%}": "e", "(ng)": "", "%{.sf.%}": "%{.sf.%}", "%{.m.%}": "%{.m.%}"}, # NounInflVII: when suffixing to noun roots that have semi-final -e
                  {"%{E%}": "", "(ng)": "ng", "%{.f.%}": "%{.f.%}"}, # NounInflVIII: when suffixing to noun roots that end in final -e that can be dropped but not hopped
...
rules_for_verb = [{"(g/t)": "%{G%}", "%{G%}":"%{G%}", "(t)":"", "(te)":""}, # VerbSuffixI: when suffixing to verb roots that end in -a, -i, -u
                  ...
                  {"(g/t)": "t", "(t)":"t", "(te)":"te", "%{E%}":"e", "%{.sf.%}%{G%}": "%{.sf.%}%{G%}", "%{.sf.%}": "%{.sf.%}", "%{.m.%}": "%{.m.%}"}, # VerbSuffixVI: when suffixing to verb roots that have semi-final -e
                  ]
